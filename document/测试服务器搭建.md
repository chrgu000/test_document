...
查看端口号
[root@host_name ~]#/etc/init.d/iptables status
检查iptables是否启动
[root@host_name ~]#service iptables status
停止iptables
[root@host_name ~]#service iptables stop

[root@host_name ~]#/sbin/iptables -I INPUT -p tcp --dport 7000  -j ACCEPT
[root@host_name ~]#/sbin/iptables -I INPUT -p tcp --dport 7001  -j ACCEPT
[root@host_name ~]#/sbin/iptables -I INPUT -p tcp --dport 7002  -j ACCEPT
[root@host_name ~]#/sbin/iptables -I INPUT -p tcp --dport 7003  -j ACCEPT
[root@host_name ~]#/sbin/iptables -I INPUT -p tcp --dport 7004  -j ACCEPT
[root@host_name ~]#/sbin/iptables -I INPUT -p tcp --dport 7005  -j ACCEPT
[root@host_name ~]#/etc/init.d/iptables save
[root@host_name ~]#/etc/init.d/iptables restart
[root@host_name ~]#/etc/init.d/iptables status
...
将下载的文件进行解压：
[root@zhufukun tmp]# tar -xvf mysql-5.7.12-1.el6.x86_64.rpm-bundle.tar
按顺序依次安装：
...
2.登录：mysql -u root -p，初次登录密码使用临时密码
...
注意：如果将my.cnf配置文件之前加的跳过密码检测内容去掉，重启服务；
